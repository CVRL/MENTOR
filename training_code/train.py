...
if args.network == "resnet152":
    ...
    map_size = 8
    im_size = 0
    if args.alpha == 1.0: # Xent
        model = timm.create_model('inception_v4', pretrained=True, num_classes=2)
        data_config = timm.data.resolve_model_data_config(model)
        transforms = timm.data.create_transform(**data_config, is_training=True)
        model = model.to(device)
        model.features[-1].branch3[-1].conv.register_forward_hook(getActivation('features'))
    else:
    ...
    model = model.to(device)
    # CYBORG activation hook
    model.features[-1][0].register_forward_hook(getActivation('features'))
