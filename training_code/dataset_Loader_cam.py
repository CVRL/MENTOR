...
class datasetLoader(data_utl.Dataset):
    ...
    def __init__(self, split_file, root, train_test, random=True, c2i={}, map_location='',map_size=7,im_size=224,network='densenet'):
        ...
        if network == "xception":
            ...
            self.data_config = timm.data.resolve_model_data_config(timm.create_model('inception_v4', pretrained=True))
            ...
            for l in tqdm(f.readlines()):
                ...
                if train_test == v[0]:
                    ...
                    if train_test == 'train' and os.path.exists(self.map_location + image_name.split("/")[-1]):
                        human_map = Image.open(self.map_location + image_name.split("/")[-1]).convert("L")
                    ...
                    #self.data.append([imagePath, self.c,transform_human_map])
                    self.data.append([imagePath, self.class_to_id[c],transform_human_map])
                    #self.data.append([imagePath, c,transform_human_map])
    ...
    def __getitem__(self, index):
        imagePath, cls, hmap = self.data[index]
        imageName = imagePath.split('/')[-1]
        ...
        return tranform_img[0:3,:,:], cls, imageName, hmap
